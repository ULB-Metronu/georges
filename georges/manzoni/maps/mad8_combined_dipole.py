...
def compute_mad_combined_dipole_tensor(element_parameters: list, global_parameters: list) -> np.ndarray:
    ...
    if k1 == 0 and h == 0:
        ...
        sxp2y = (2 * np.sin(((2 * np.sqrt(-k1) + np.sqrt(h**2 + k1)) * L) / 2)) / (
            2 * np.sqrt(-k1) + np.sqrt(h**2 + k1)
        )
        sxm2y = (2 * np.sin(((-2 * np.sqrt(-k1) + np.sqrt(h**2 + k1)) * L) / 2)) / (
            -2 * np.sqrt(-k1) + np.sqrt(h**2 + k1)
        )
        ...
        fxp2y = (
            ...
            * (
                L
                - (2 * np.sin(((2 * np.sqrt(-k1) + np.sqrt(h**2 + k1)) * L) / 2))
                / (2 * np.sqrt(-k1) + np.sqrt(h**2 + k1))
            )
    ...
    T[0, 1, 5] = (
        -(cx * L + sx) / (4 * beta)
        + (h**2 * (cx * j1 + dx * sx)) / (4 * beta)
        - (h * (2 * h * k1 + k2) * (-2 * cx * j2 + dx**2 * sx)) / (12 * beta)
    )
    T[0, 5, 5] = (
        (h**3 * j1 * sx) / (2 * beta**2)
        - (h * L * sx) / (2 * beta**2)
        - (h**2 * (2 * h * k1 + k2) * (dx**3 - 2 * j2 * sx)) / (6 * beta**2)
        - (dx * h) / (2 * beta**2 * gamma**2)
    )
    ...
    T[1, 5, 5] = (
        -(h * k1 * (cx * j1 - dx * sx)) / (2 * beta**2)
        - (h**2 * (2 * h * k1 + k2) * (-2 * cx * j2 + dx**2 * sx)) / (6 * beta**2)
        - (h * sx) / (2 * beta**2 * gamma**2)
    )
    ...
    T[2, 3, 5] = (
        (cy * h**2 * j1) / (2 * beta) - (cy * L + sy) / (4 * beta) + (h * k2 * (-2 * cy * jf + jd * sy)) / (2 * beta)
    )
    ...
    T[3, 2, 5] = (
        (cy * h * j1 * (h * k1 + k2)) / (2 * beta)
        + (k1 * (-(cy * L) + sy)) / (4 * beta)
        + (h * k1 * k2 * (2 * cy * jf - jd * sy)) / (2 * beta)
    )
    T[3, 3, 5] = (
        (h * j1 * (h * k1 + k2) * sy) / (2 * beta)
        - (k1 * L * sy) / (4 * beta)
        + (h * k2 * (-(cy * jd) + 2 * jf * k1 * sy)) / (2 * beta)
    )
    ...
    T[4, 0, 5] = (
        ((1 + cx) * h * j1 * k1) / (4 * beta**2)
        + (h**2 * (3 * dx * j1 - 4 * j2) * (2 * h * k1 + k2)) / (12 * beta**2)
        + (h * sx) / (2 * beta**2 * gamma**2)
    )
    T[4, 1, 5] = (
        (h * j1 * k1 * sx) / (4 * beta**2)
        + (h**2 * (2 * h * k1 + k2) * (dx**3 - 2 * j2 * sx)) / (12 * beta**2)
        + (dx * h) / (2 * beta**2 * gamma**2)
    )
    T[4, 5, 5] = (
        (h**3 * (-2 * dx * j2 + 3 * j3) * (2 * h * k1 + k2)) / (6 * beta**3)
        + (h**2 * k1 * (-((1 + 2 * cx) * j2) + dx**2 * sx)) / (6 * beta**3)
        + (3 * (h**2 * j1 - L)) / (2 * beta**3 * gamma**2)
    )
    T[4, 2, 2] = (
        -((h * jf * k1 * k2) / beta) - (h * j1 * (h * k1 + k2)) / (2 * beta) + (k1 * (L - cy * sy)) / (4 * beta)
    )
