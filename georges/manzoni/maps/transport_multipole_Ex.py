...
def compute_transport_multipole_Ex_tensor(element_parameters: nList) -> np.ndarray:
    ...
    if k1 < 0:
        ...
        T[2, 0, 2] = 2 * k2 * (
            ...
            L * np.sqrt(-k1)) + np.cos(L * np.sqrt(-k1))) / (5 * k1)
        T[2, 0, 3] = -2 * k2 * (
                np.sin(L * np.sqrt(-k1)) * np.cosh(L * np.sqrt(-k1)) + np.sin(L * np.sqrt(-k1)) - 2 * np.cos(
            L * np.sqrt(-k1)) * np.sinh(L * np.sqrt(-k1))) / (5 * (-k1) ** (3 / 2))
        T[2, 1, 2] = -2 * k2 * (
        ...
        T[2, 1, 3] = -2 * k2 * (
            ...
            L * np.sqrt(-k1)) - 2 * np.cos(L * np.sqrt(-k1))) / (5 * k1 ** 2)
        ...
        T[3, 0, 2] = 2 * k2 * (
            ...
            L * np.sqrt(-k1)) * np.sinh(L * np.sqrt(-k1))) / (5 * np.sqrt(-k1))
        T[3, 0, 3] = 2 * k2 * (
