...
@njit
def compute_transport_quadrupole_matrix(L: float, K1: float, K2: float) -> np.ndarray:
    kq2 = K1
    R = np.zeros((6, 6))
...
@njit
def compute_transport_quadrupole_tensor(L: float, K1: float, K2: float) -> np.ndarray:
    kq2 = K1
    T = np.zeros((6, 6, 6))
    ...
    T[3, 2, 5] = kq2 * L / 2 * cq(-kq2, L) + kq2 * sq(-kq2, L)
