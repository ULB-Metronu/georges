...
def compute_transport_combined_dipole_tensor(element_parameters: nList) -> np.ndarray:
    ...
    if h != 0:
        if k1 == 0:
            ...
            T[0, 5, 5] = (-12*L*h*sin(L*h) + 6*sin(L*h)**6 - 18*sin(L*h)**4 + 19*sin(L*h)**2 + 6*cos(L*h)**6 - 16*cos(L*h) + 10)/(6*h)
            T[1, 0, 0] = -h**2*sin(2*L*h)/2
            T[1, 0, 1] = h*(-cos(L*h) + cos(2*L*h))
            T[1, 0, 5] = h*sin(2*L*h)
            T[1, 1, 1] = -sin(L*h)/2 + sin(2*L*h)/2
            T[1, 1, 5] = 2*L*h*sin(L*h) + 2*cos(L*h)**2/3 - cos(L*h)/3 - 1/3
            T[1, 3, 3] = -sin(L*h)/2
            T[1, 5, 5] = -2*L*h*cos(L*h) + sin(L*h)*cos(L*h)/3 + 2*sin(L*h)/3
            T[2, 0, 3] = -L*h + sin(L*h)
            T[2, 1, 3] = (1 - cos(L*h))/h
            T[2, 3, 5] = L - sin(L*h)/h
            T[3, 0, 3] = h*(cos(L*h) - 1)
