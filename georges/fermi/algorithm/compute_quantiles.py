...
def compute_quantile_2dim(data: pd.DataFrame = None) -> Tuple[float, pd.DataFrame]:
    ...
    quantile = minimize(lambda xi: get_deviation_2dim(data=beam,
                                                      quant=xi),
                        0, method='Nelder-Mead', tol=1e-8)
...
def compute_chisquare_2dim(data_x: np.array = None,
                           data_y: np.array = None, ) -> float:
...
def compute_quantile_4dim(beam: pd.DataFrame = None) -> Tuple[float, pd.DataFrame]:
    ...
    # Compute the quantile
    quantile = minimize(lambda xi: get_deviation_4dim(data=beam,
                                                      quant=xi),
                        0, method='Nelder-Mead', tol=1e-8, options={"disp": False, 'fatol': 1e-12})
...
def compute_chisquare_4dim(data_x: np.array = None,
                           data_xp: np.array = None,
                           data_y: np.array = None,
                           data_yp: np.array = None, ) -> float:
    """ Computes the chi-square distribution """
...
def get_deviation_4dim(data: pd.DataFrame = None, quant=None) -> float:
    ...
    deviation_chisquare = np.abs((compute_chisquare_4dim(data_x,
                                                         data_xp,
                                                         data_y,
                                                         data_yp)) - 8.0)
    return deviation_chisquare
