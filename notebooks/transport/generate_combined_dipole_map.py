...
from numba.typed import List as nList 
    \n\n""")
        ...
        if exact_dpp:
            ...
            f.write(f"""def compute_transport_combined_dipole_matrix(element_parameters: nList) -> np.ndarray:\n""")
        f.write("""
    ...
    R[5, 5] = 1 \n""")
        ...
        if exact_dpp:
            ...
            f.write(f"""def compute_transport_combined_dipole_tensor(element_parameters: nList) -> np.ndarray:\n""")
        f.write("""
    L: float = element_parameters[0]
    alpha: float = element_parameters[1]
    h = alpha / L
