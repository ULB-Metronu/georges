...
from numba.typed import List as nList
    \n\n""")
        ...
        if exact_dpp:
            f.write(f"""def compute_transport_quadrupole_ex_matrix(element_parameters: nList) -> np.ndarray:\n""")
        else:
            f.write(f"""def compute_transport_quadrupole_matrix(element_parameters: nList) -> np.ndarray:\n""")
        f.write("""
    ...
    R[5, 5] = 1 \n""")
        ...
        f.write(f"@njit(parallel=True, fastmath=True)\n")
        if exact_dpp:
            f.write(f"""def compute_transport_quadrupole_ex_tensor(element_parameters: nList) -> np.ndarray:\n""")
        else:
            f.write(f"""def compute_transport_quadrupole_tensor(element_parameters: nList) -> np.ndarray:\n""")
        f.write("""
    L: float = element_parameters[0]
    k1: float = element_parameters[1]""")
        if exact_dpp:
