...
from numba.typed import List as nList
    \n\n""",
        ...
        if exact_dpp:
            f.write("""def compute_transport_multipole_ex_matrix(element_parameters: nList) -> np.ndarray:\n""")
        else:
            f.write("""def compute_transport_multipole_matrix(element_parameters: nList) -> np.ndarray:\n""")
        f.write(
            """
    ...
    R[5, 5] = 1 \n""",
        ...
        f.write("@njit(cache=True)\n")
        if exact_dpp:
            f.write("""def compute_transport_multipole_ex_tensor(element_parameters: nList) -> np.ndarray:\n""")
        else:
            f.write("""def compute_transport_multipole_tensor(element_parameters: nList) -> np.ndarray:\n""")
        f.write(
            """
    L: float = element_parameters[0]
    k1: float = element_parameters[1]
